...
from .models import VMInfo, Registration, Metric,Employee,VmRequest,CdacProject, ServiceRequest,Ticket
...
class EmployeeSerializer(serializers.ModelSerializer):
    class Meta:
        model = Employee
        fields = ['name', 'employee_id', 'email', 'group', 'fla_name',"fla_email","fla_employee_id"]
...
class TicketSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = [
            ...
            'solution', 'status', 'created_at', 'status_updated_at', 'closed_at'
        ]
        read_only_fields = [
            'employee_name', 'employee_email', 'employee_id',
            'created_at', 'status_updated_at', 'closed_at'
        ]
...
class CustomTokenObtainPairSerializer(TokenObtainPairSerializer):
    ...
    def get_token(cls, user):
        print("===", user)
        emp_id = Employee.objects.filter(email=user.email).values_list('employee_id', flat=True)
        print("===", emp_id, "=======")
...
class VmRequestSerializer(serializers.ModelSerializer):
    ...
    def validate_vm_name(self, value):
        ...
        if VmRequest.objects.filter(vm_name=value).exists():
            raise serializers.ValidationError("A VM with this name already exists. Please choose a different name.")
        return value
