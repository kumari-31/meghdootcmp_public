...
const Networks = () => {
  ...
  const handleDeleteSelectedNetworks = async () => {
    ...
    if (!window.confirm('Are you sure you want to delete the selected networks?')) {
      return;
    }
  ...
  const handleDeleteNetwork = async (networkId) => {
    ...
    try {
      ...
      if (error.response) {
        ...
        alert(`Failed to delete the network: ${error.response.data.message || 'Unknown error'}`);
      } else {
          ...
          <TableHead>
            <TableRow>
              <StyledTableCell padding="checkbox">
                <input
                  type="checkbox"
                  checked={filteredNetworks.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage)
