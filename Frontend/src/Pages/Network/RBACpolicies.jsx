...
const RBACpolicies = () => {
  ...
  const createRbacPolicy = async (e) => {
    ...
    try {
      ...
      if (response.status === 200 || response.status === 201) {
        ...
        showSnackbar(
          "Failed to create RBAC Policy: Unexpected response.",
          "error"
        );
      }
    } catch (error) {
      ...
      if (error.response) {
        ...
        if (serverError.quota_exceeded) {
          ...
          showSnackbar(
            `Failed to create RBAC Policy: ${serverError.error}`,
            "error"
          );
        } else {
          showSnackbar(
            "Failed to create RBAC Policy. Please try again.",
            "error"
          );
        }
      } else {
  ...
  const updateRbacPolicy = async (e) => {
    ...
    try {
      ...
      const response = await apiClient.patch(
        `/update-rbac-policy/${editingPolicy.id}/`,
        payload
      );
      if (response.status === 200) {
        ...
        showSnackbar(
          "Failed to update RBAC Policy: Unexpected response.",
          "error"
        );
      }
    } catch (error) {
      ...
      if (error.response && error.response.data && error.response.data.error) {
        showSnackbar(
          `Failed to update RBAC Policy: ${error.response.data.error}`,
          "error"
        );
      } else {
        showSnackbar(
          "Failed to update RBAC Policy. Please try again.",
          "error"
        );
      }
    } finally {
  ...
  const deleteRbacPolicy = async (policyId) => {
    ...
    try {
      ...
      if (response.status === 200 || response.status === 204) {
        ...
        showSnackbar(
          "Failed to delete RBAC Policy: Unexpected response.",
          "error"
        );
      }
    } catch (error) {
      ...
      if (error.response && error.response.data && error.response.data.error) {
        showSnackbar(
          `Failed to delete RBAC Policy: ${error.response.data.error}`,
          "error"
        );
      } else {
        showSnackbar(
          "Failed to delete RBAC Policy. Please try again.",
          "error"
        );
      }
    }
  ...
  const handleDeleteSelectedPolicies = async () => {
    ...
    if (
      !window.confirm("Are you sure you want to delete the selected policies?")
    ) {
      return;
    }
    ...
    try {
      ...
      for (const policyId of selectedPolicies) {
        try {
          ...
          if (response.status === 200 || response.status === 204) {
            ...
            console.warn(
              `Failed to delete policy ${policyId}: Unexpected response`,
              response
            );
          }
        } catch (innerError) {
      ...
      if (successfulDeletes === selectedPolicies.length) {
        ...
        showSnackbar(
          `Successfully deleted ${successfulDeletes} out of ${selectedPolicies.length} policies. Some deletions failed.`,
          "warning"
        );
      } else {
      ...
      console.error(
        "An unexpected error occurred while deleting selected policies:",
        error
      );
      showSnackbar(
        "An unexpected error occurred while deleting selected policies.",
  ...
  return (
    <div style={rbacContainerStyle}>
        ...
        <Box sx={modalStyle}>
          ...
          <form onSubmit={createRbacPolicy}>
            <FormControl
              fullWidth
              margin="normal"
              error={!!formErrors.targetProject}
            >
            ...
            <FormControl
              fullWidth
              margin="normal"
              error={!!formErrors.objectType}
            >
              <InputLabel id="objectType-label">
      ...
      <Modal
        ...
        onClose={updating ? undefined : () => setEditingPolicy(null)}
      >
        <Box sx={modalStyle}>
          <Typography variant="h6" component="h2" gutterBottom>
            Update RBAC Policy
          ...
          <form onSubmit={updateRbacPolicy}>
            <FormControl fullWidth margin="normal">
              <InputLabel id="targetProjectUpdate-label">
                Target Project
              </InputLabel>
              <Select
                ...
                <input
                  ...
                  checked={
                    ...
                    filteredPolicies
                      ...
                      .every((policy) => selectedPolicies.includes(policy.id))
                  }
                />
              </StyledTableCell>
              ...
              : filteredPolicies
                  ...
                  .map((policy, index) => (
                    <StyledTableRow key={policy.id}>
                      ...
                      <StyledTableCell>{policy.owner_project}</StyledTableCell>

                      <StyledTableCell>{policy.id}</StyledTableCell>

                      <StyledTableCell>{policy.object_type}</StyledTableCell>

                      <StyledTableCell>{policy.target_project}</StyledTableCell>
...
const headerContainerRbacStyle = {
  display: "flex",
